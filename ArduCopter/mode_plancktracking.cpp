    ...
    if(copter.planck_interface.new_command_available()) {
        switch(copter.planck_interface.get_cmd_type()) {
          ...
          case copter.planck_interface.ACCEL:
          {
              ...
              Vector3f ang_vel{};
              if(is_yaw_rate) {
                ang_vel.z = yaw_rate_rads;
              }
              ModeGuided::set_angle(q,ang_vel,vz_cms,false);
              break;
          }
...
bool ModePlanckTracking::allows_arming(AP_Arming::Method method) const
{
    if(method != AP_Arming::Method::MAVLINK) return false;
    if(!copter.planck_interface.ready_for_takeoff())
    {
